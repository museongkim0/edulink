        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="dt in dt_resp.boardList" :key="dt.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white cursor-pointer text-center" @click="openPopup(dt.idx)" style="width: 20%;">{{ dt.title }}</th>
                    <td class="px-6 py-4 cursor-pointer text-center" @click="openPopup(dt.idx)" style="width: 30%;">{{ dt.fileName }}</td>
                    <td class="px-6 py-4 cursor-pointer text-center" @click="openPopup(dt.idx)" style="width: 15%;">{{ new Date(dt.createdDate).toLocaleDateString() }}</td>
                    <td class="px-6 py-4 cursor-pointer text-center" @click="openPopup(dt.idx)" style="width: 15%;">{{ dt.writer }}</td>
                    <td class="px-6 py-4 text-center" style="width: 15%;">
                        ...
                        <button @click="openPopup2(dt.idx)" class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600" type="button">
                            <svg class="w-4 h-5 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24">
